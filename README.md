...
`Key_uint64`, `Key_uint32`, `Key_uint16`, `Key_uint8`, `Key_string`, `Key_bytes`, `Key_runes`

Numeric keys are much faster than `string`, `[]byte` or `[]rune`. `Key_bytes` and `Key_runes` are the most highly optimized and are twice as fast as `Key_string`, so the only reason to ever use `Key_string` over `Key_bytes` is if you care specifically about the order that the keys are stored in.
...
`MyStruct.AddKeyUnsorted(key)` appends the key to the end, unsorted, which means you should not search the keys until you call `MyStruct.Build()`. This is more efficient if you intend to add all keys in one go and you already know they are unique.
...
However, I strongly recommend not to use interpolation search unless you are certain that your keys are evenly distributed. Interpolation search is only faster when keys are evenly distributed, but can be much, much slower if they are not. If you're unsure just use `Find()`. If you are using `Key_string`, `Key_bytes` or `Key_runes` then interpolation search is not available anyway. I also never tested `FindInterpolation()` with `AddKeyAt()`, so it's probably safer to only use `Find()` to retrieve the indexes for `AddKeyAt()`.
